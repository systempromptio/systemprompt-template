...
use crate::fixtures::{
    AclRuleSpec, insert_acl_rule, insert_department, insert_user, insert_user_full, set_department,
...
async fn list_department_names_includes_a_new_department() {
    ...
    insert_department(&db.pool, &unique("dept"), &name).await;
...
async fn list_user_management_aggregates_counts_the_grants_a_user_receives() {
    ...
    insert_acl_rule(
        &db.pool,
        &AclRuleSpec::allow("skill", &unique("skill"), "user", user.as_str()),
    )
    .await;
    insert_acl_rule(
        &db.pool,
        &AclRuleSpec::deny("skill", &unique("skill"), "user", user.as_str()),
    )
    .await;
...
async fn list_user_marketplace_overrides_reports_both_scopes() {
    ...
    insert_acl_rule(
        &db.pool,
        &AclRuleSpec::allow("marketplace", &by_department, "department", &name),
    )
    .await;
    insert_acl_rule(
        &db.pool,
        &AclRuleSpec::deny("marketplace", &by_user, "user", user.as_str()),
    )
    .await;
