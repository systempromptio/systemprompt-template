...
async fn get_request_stats_is_all_zeroes_in_an_empty_window() {
    ...
    let stats = get_request_stats(&db.pool, narrow_window(), &SubjectScope::All)
        .await
...
async fn list_latency_histogram_always_returns_every_bin() {
    ...
    let buckets = list_latency_histogram(&db.pool, narrow_window(), &SubjectScope::All)
        .await
        .expect("query succeeds");
...
async fn list_request_timeseries_returns_a_fixed_number_of_buckets() {
    ...
    let series = list_request_timeseries(&db.pool, narrow_window(), &SubjectScope::All)
        .await
        .expect("query succeeds");
