...
async fn list_unencrypted_secrets_skips_non_secret_and_empty_rows() {
    ...
    let user = unique("u");
    insert_user(&db.pool, &user).await;
    insert_env_var(&db.pool, &user, "plug", "PUBLIC", "visible").await;
    insert_secret_env_var(&db.pool, &user, "plug", "BLANK", "").await;
...
async fn update_encrypted_value_clears_the_plaintext_column() {
    ...
    let user = unique("u");
    insert_user(&db.pool, &user).await;
    let id = insert_secret_env_var(&db.pool, &user, "plug", "LEGACY", "plaintext").await;
