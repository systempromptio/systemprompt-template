...
fn validate_input(input: &CreateMcpServerInput) -> Result<(), McpError> {
...
) -> systemprompt_web_extension::admin::types::CreateUserMcpServerRequest {
    systemprompt_web_extension::admin::types::CreateUserMcpServerRequest {
        mcp_server_id: systemprompt::identifiers::McpServerId::new(mcp_server_id),
        name: input.name.clone(),
        description: input.description.clone(),
        binary: input.binary.clone().unwrap_or_default(),
        package_name: input.package_name.clone().unwrap_or_default(),
        port: input.port,
        endpoint: input.endpoint.clone(),
        oauth_required: input.oauth_required,
        oauth_scopes: input.oauth_scopes.clone(),
        oauth_audience: input.oauth_audience.clone().unwrap_or_default(),
...
impl McpToolHandler for CreateMcpServerHandler {
    ...
    async fn handle(
        ...
        let create_req = build_create_request(&input, mcp_server_id);
        ...
        build_response(&server, added_to_plugin.as_ref(), ctx)
    }
}
