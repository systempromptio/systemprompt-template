...
fn serialize_json<T: serde::Serialize>(value: &T, label: &str) -> Result<String, McpError> {
    serde_json::to_string(value).map_err(|e| {
        McpError::internal_error(format!("Serialization failed for {label}: {e}"), None)
    })
}

fn build_plugin_row(
    ep: &systemprompt_web_extension::admin::repositories::user_plugins::UserPluginEnriched,
    onboarding_configs: &std::collections::HashMap<
        String,
        systemprompt_web_extension::admin::types::PluginOnboardingConfig,
    >,
    ...
    let onboarding_value = match onboarding {
        Some(o) => serialize_json(o, "onboarding config")?,
        None => String::new(),
    };
    ...
    Ok(serde_json::json!({
        ...
        "skills": serialize_json(&skills, "skills")?,
        "agents": serialize_json(&agents, "agents")?,
        "mcp_servers": serialize_json(&mcp_servers, "mcp_servers")?,
...
impl McpToolHandler for ListPluginsHandler {
    ...
    async fn handle(
        ...
        let rows: Vec<serde_json::Value> = enriched_plugins
            .iter()
            .map(|ep| build_plugin_row(ep, &onboarding_configs))
            .collect::<Result<Vec<_>, McpError>>()?;

        let total = rows.len();
        let summary = format!("Found {total} plugin(s) for user '{user_id}'");
        let artifact = TableArtifact::new(build_columns(), ctx).with_rows(rows);
