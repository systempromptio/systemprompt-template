        ...
        tokio::try_join!(
            async {
                if !input.skill_ids.is_empty() {
                    let uuids =
                        shared::resolve_skill_slugs(&pool, user_id.as_ref(), &input.skill_ids)
                            .await?;
            ...
            async {
                if !input.agent_ids.is_empty() {
                    let uuids =
                        shared::resolve_agent_slugs(&pool, user_id.as_ref(), &input.agent_ids)
                            .await?;
            ...
            async {
                if !input.mcp_server_ids.is_empty() {
                    let uuids = shared::resolve_mcp_server_slugs(
                        &pool,
                        user_id.as_ref(),
