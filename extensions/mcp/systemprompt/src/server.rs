    ...
    fn get_info(&self) -> ServerInfo {
        ...
        .with_instructions(
            "Execute SystemPrompt CLI commands. MANDATORY FIRST STEP: Run 'core playbooks show guide_start' \
             ...
             Discord: 'plugins run discord send \"message\"'. Full documentation: https://foodles.com/playbooks",
        )
    }
...
async fn record_mcp_access_rejected(pool: &DbPool, server: &str, tool: &str, reason: &str) {
    let Some(pg_pool) = pool.pool() else {
