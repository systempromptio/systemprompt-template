-- Consolidated schema: Campaign links and analytics
...
SELECT
    cl.id, cl.short_code, cl.target_url, cl.link_type, cl.campaign_id, cl.campaign_name,
    cl.source_content_id, cl.source_page, cl.link_text, cl.destination_type,
    cl.click_count, cl.unique_click_count, cl.conversion_count, cl.is_active,
    ...
    MIN(lc.clicked_at) as first_click_at, MAX(lc.clicked_at) as last_click_at,
    AVG(lc.time_on_page_seconds) as avg_time_on_page, AVG(lc.scroll_depth_percent) as avg_scroll_depth
...
SELECT
    cl.campaign_id, cl.campaign_name,
    COUNT(DISTINCT cl.id) as link_count,
    SUM(cl.click_count) as total_clicks, SUM(cl.unique_click_count) as total_unique_clicks,
    SUM(cl.conversion_count) as total_conversions,
    COUNT(DISTINCT lc.session_id) as total_sessions, COUNT(DISTINCT lc.user_id) as total_users,
    ROUND(100.0 * SUM(cl.conversion_count) / NULLIF(SUM(cl.unique_click_count), 0), 2) as conversion_rate,
    MIN(cl.created_at) as campaign_start, MAX(lc.clicked_at) as last_activity,
...
SELECT
    cl.source_content_id, mc_source.slug as source_slug, mc_source.title as source_title,
    cl.target_url, mc_target.slug as target_slug, mc_target.title as target_title,
    COUNT(lc.id) as click_count, COUNT(DISTINCT lc.session_id) as unique_sessions,
    ...
    MIN(lc.clicked_at) as first_journey_at, MAX(lc.clicked_at) as last_journey_at
FROM campaign_links cl
LEFT JOIN link_clicks lc ON cl.id = lc.link_id
...
WHERE cl.source_content_id IS NOT NULL AND cl.destination_type = 'internal'
...
SELECT
    lc.id as click_id, lc.clicked_at, cl.short_code, cl.campaign_name,
    cl.source_page, cl.target_url, lc.session_id, lc.user_id,
    lc.device_type, lc.country, lc.is_first_click, lc.is_conversion, lc.referrer_page
FROM link_clicks lc
...
SELECT
    cl.id, cl.short_code, cl.target_url, cl.campaign_name, cl.link_type,
    cl.click_count, cl.unique_click_count, cl.conversion_count,
