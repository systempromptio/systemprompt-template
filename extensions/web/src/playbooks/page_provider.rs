...
fn extract_basic_fields(item: &Value, data: &mut serde_json::Map<String, Value>) {
    ...
    if let Some(slug) = item.get("slug").and_then(|v| v.as_str()) {
        ...
        let category = PlaybookPageDataProvider::derive_category(slug);
...
fn extract_keywords(item: &Value, data: &mut serde_json::Map<String, Value>) {
...
fn extract_dates(item: &Value, data: &mut serde_json::Map<String, Value>) {
...
fn extract_priority(item: &Value, data: &mut serde_json::Map<String, Value>) {
...
fn extract_related_playbooks(item: &Value, data: &mut serde_json::Map<String, Value>) {
    if let Some(related) = item.get("related_playbooks").and_then(|v| v.as_array()) {
        if !related.is_empty() {
            ...
            if let Some(html) = PlaybookPageDataProvider::render_related_playbooks(related) {
        ...
        .and_then(|v| v.as_array())
    {
        if !same_cat.is_empty() {
            ...
            if let Some(html) = PlaybookPageDataProvider::render_same_category_playbooks(same_cat) {
                data.insert(
...
impl PageDataProvider for PlaybookPageDataProvider {
    ...
    async fn provide_page_data(&self, ctx: &PageContext<'_>) -> Result<Value> {
        ...
        extract_basic_fields(item, &mut data);
        extract_keywords(item, &mut data);
        extract_dates(item, &mut data);
        extract_priority(item, &mut data);
        extract_related_playbooks(item, &mut data);
