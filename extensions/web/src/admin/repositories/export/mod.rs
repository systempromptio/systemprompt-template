...
pub async fn generate_export_bundles(
    ...
    let plugin_tokens: std::collections::HashMap<String, String> = plugin_configs
        .iter()
        .filter_map(|(id, _)| {
            super::plugin_jwt::generate_plugin_token(params.user_id, params.email, id)
                .map(|t| (id.clone(), t))
...
fn filter_forked_plugins(
    all_configs: Vec<(String, crate::admin::types::PlatformPluginConfig)>,
    user_plugins: &[crate::admin::types::UserPlugin],
) -> Vec<(String, crate::admin::types::PlatformPluginConfig)> {
...
fn generate_bundle_env_and_hook_files(
    ...
    for bundle in bundles.iter_mut() {
        if let Some(token) = tokens.get(&bundle.id) {
            ...
            if let Err(e) =
                super::export_builders::build_hook_files(platform_url, token, &mut bundle.files)
            {
...
fn build_org_bundles(
    plugin_configs: &[(String, crate::admin::types::PlatformPluginConfig)],
    plugins_path: &Path,
    skills_path: &Path,
    services_path: &Path,
    platform_url: &str,
...
pub async fn generate_org_marketplace_export_bundles(
    ...
    let plugin_ids = super::org_marketplaces::list_marketplace_plugin_ids(pool, marketplace_id)
        .await
        .map_err(|e| MarketplaceError::Internal(format!("Failed to list marketplace plugins: {e}")))?;

    let plugins_path = services_path.join("plugins");
    let skills_path = services_path.join("skills");
    let platform_url = Config::get().map_or_else(|_| String::new(), |c| c.api_external_url.clone());

    let all_configs = load_plugin_configs_by_ids(&plugins_path, &plugin_ids.iter().cloned().collect())?;
    let tokens = std::collections::HashMap::new();

    let bundles = build_org_bundles(&all_configs, &plugins_path, &skills_path, services_path, &platform_url, &tokens)?;
    let totals = compute_export_totals(&bundles);
