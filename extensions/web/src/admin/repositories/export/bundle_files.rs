...
pub async fn build_skill_files(
    ...
    skill: &crate::admin::types::UserSkill,
    ctx: &BundleContext<'_>,
) {
...
pub fn build_agent_file(files: &mut Vec<PluginFile>, agent: &crate::admin::types::UserAgent) {
...
pub fn build_mcp_files(
    ...
    all_user_mcp_servers: &[crate::admin::types::UserMcpServer],
    platform_url: &str,
...
pub fn build_env_and_hook_files_with_token(
    ...
    let token = ctx
        ...
        .map(String::from)
        .or_else(|| {
            super::super::plugin_jwt::generate_plugin_token(ctx.user_id, email, ctx.plugin_id)
    ...
    if let Some(ref token) = token {
        ...
        if let Err(e) =
            super::super::export_builders::build_hook_files(ctx.platform_url, token, files)
        {
