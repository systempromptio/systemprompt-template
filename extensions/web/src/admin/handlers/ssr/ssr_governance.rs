...
pub(crate) async fn governance_page(
    ...
    let rows = repositories::dashboard::list_governance_decisions(&pool, search)
        .await
        .unwrap_or_else(|e| {
    ...
    let secret_breaches: i64 = rows.iter().filter(|r| r.policy == "secret_injection").count() as i64;

    let decisions_json: Vec<serde_json::Value> = rows
        .iter()
        .map(|r| {
            json!({ // JSON: protocol boundary
                "id": r.id,
                "user_id": r.user_id,
    ...
    let data = json!({ // JSON: protocol boundary
        "page": "governance",
        "title": "Governance",
        "total": total,
        "denied": denied,
