...
struct AchievementMaps<'a> {
    ...
    stats: std::collections::HashMap<&'a str, &'a crate::admin::types::AchievementInfo>,
}
...
fn build_category_achievements(
    ...
    cat_defs: &[&crate::admin::gamification::AchievementDef],
...
fn build_categories(
    maps: &AchievementMaps<'_>,
    defs: &[crate::admin::gamification::AchievementDef],
...
fn compute_xp_progress(profile: Option<&crate::admin::types::UserGamificationProfile>) -> i64 {
    profile.map_or(0, |p| {
...
fn build_ranks(user_rank_level: i32) -> (Vec<RankView>, bool, bool) {
    let all_ranks = crate::admin::gamification::constants::RANKS;
...
async fn load_achievements_data(
    ...
    Option<crate::admin::types::UserGamificationProfile>,
) {
    let stats = crate::admin::gamification::queries::get_achievement_stats(pool)
        .await
        .unwrap_or_else(|e| {
...
pub(crate) async fn achievements_page(
    ...
    let max_rank =
        i32::try_from(crate::admin::gamification::constants::RANKS.len()).unwrap_or(i32::MAX);
