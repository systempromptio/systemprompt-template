...
pub(crate) async fn skills_page(
    ...
    let skills_data: Vec<serde_json::Value> = skills
        ...
        .map(|skill| {
            ...
            let agent_count = if let Some(plugins) = skill_plugin_map.get(&skill.skill_id) {
            ...
            let source = if skill.source_id == "custom" || skill.source_id == "user" {
                "custom"
            } else {
                "system"
            };
            ...
            let usage_count = usage_counts.get(&skill.skill_id).copied().unwrap_or(0);
...
pub(crate) async fn skill_edit_page(
    ...
    let skill = if let Some(id) = skill_id {
        repositories::get_agent_skill(&pool, id)
            .await
            .map_err(|e| {
