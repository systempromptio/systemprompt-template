...
use crate::admin::types::marketplaces::OrgMarketplace;
...
fn build_plugin_details(services_path: &Path, plugin_ids: &[String]) -> PluginCounts {
...
pub async fn org_marketplace_page(
    ...
    let rules_map = build_rules_map(&all_rules);
    ...
    let mut plugin_map = fetch_plugin_map(pool.as_ref()).await;
    ...
    for mkt in &marketplaces {
        ...
        let counts = build_plugin_details(&services_path, &plugin_ids);
        total_plugins += plugin_ids.len();
        total_skills += counts.skill_count;

        let key = ("marketplace".to_string(), mkt.id.clone());
        let entity_rules = rules_map.get(&key);
        marketplaces_json.push(build_marketplace_json(
            mkt,
            &plugin_ids,
            &counts,
            entity_rules,
