    ...
    let activity = match params.event_type {
        "claude_code_SessionStart" => {
            ...
            let source = params
                ...
                .source
                .as_deref()
                .or_else(|| extra.get("source").and_then(|v| v.as_str()));
            NewActivity::session_started_rich(
                params.user_id,
                params.session_id,
                model,
                project,
        ...
        "claude_code_SessionEnd" => {
            let reason = params
                ...
                .reason
                .as_deref()
                .or_else(|| extra.get("reason").and_then(|v| v.as_str()));
        ...
        "claude_code_UserPromptSubmit" => {
            let prompt = params
                ...
                .prompt
                .as_deref()
                .or_else(|| extra.get("prompt").and_then(|v| v.as_str()));
        ...
        "claude_code_PostToolUseFailure" => {
            ...
            let error = params
                ...
                .error
                .as_deref()
                .or_else(|| extra.get("error").and_then(|v| v.as_str()));
        ...
        "claude_code_Stop" => {
            let msg = params
                ...
                .as_deref()
                .or_else(|| extra.get("last_assistant_message").and_then(|v| v.as_str()));
        ...
        "claude_code_SubagentStart" => {
            let agent_type = params
                ...
                .agent_type
                .as_deref()
                .or_else(|| extra.get("agent_type").and_then(|v| v.as_str()));
...
fn spawn_plugin_installation_tracking(params: &ActivityRecordingParams<'_>) {
    let plugin_version = params
        .payload
        .extra
        .get("plugin_version")
        .and_then(|v| v.as_str());
    if let (Some(pid), Some(version)) = (params.plugin_id, plugin_version) {
        let base_plugin_id = params
            .payload
            .extra
            .get("base_plugin_id")
            .and_then(|v| v.as_str());
        let plugin_source = if base_plugin_id.is_some() {
            "custom"
        } else {
            "org"
        };
...
fn extract_tool_detail(tool: &str, tool_input: Option<&serde_json::Value>) -> String {
    ...
    match tool {
        "Bash" => input.get("command").and_then(|v| v.as_str()).map_or_else(
            || "ran a command".into(),
            |cmd| {
        ...
        "Read" => input.get("file_path").and_then(|v| v.as_str()).map_or_else(
            || "read a file".into(),
            |p| format!("read {}", short_path(p)),
        ),
        "Write" => input.get("file_path").and_then(|v| v.as_str()).map_or_else(
            || "wrote a file".into(),
            |p| format!("wrote {}", short_path(p)),
        ),
        "Edit" | "MultiEdit" => input.get("file_path").and_then(|v| v.as_str()).map_or_else(
            || "edited a file".into(),
            |p| format!("edited {}", short_path(p)),
        ),
        "Grep" => input.get("pattern").and_then(|v| v.as_str()).map_or_else(
            || "searched files".into(),
            |pat| {
                let t = if pat.len() > 40 {
                    format!("{}...", &pat[..37])
        ...
        "Glob" => input.get("pattern").and_then(|v| v.as_str()).map_or_else(
            || "found files".into(),
            |pat| {
                let t = if pat.len() > 40 {
                    format!("{}...", &pat[..37])
        ...
        "WebSearch" => input.get("query").and_then(|v| v.as_str()).map_or_else(
            || "searched web".into(),
            |q| {
                let t = if q.len() > 50 {
                    format!("{}...", &q[..47])
        ...
        "WebFetch" => input.get("url").and_then(|v| v.as_str()).map_or_else(
            || "fetched URL".into(),
            |u| {
                let t = if u.len() > 50 {
                    format!("{}...", &u[..47])
