...
use crate::admin::handlers::ssr::{
    build_session_groups_with_status, ssr_control_center::enrichment,
};
use crate::admin::repositories::{control_center, session_analyses};
use crate::admin::types::control_center::RecentSession;

use super::SessionGroup;

type SessionEntityLink = crate::admin::types::conversation_analytics::SessionEntityLink;
type SessionRating = crate::admin::types::conversation_analytics::SessionRating;
...
pub(super) async fn build_activity_event(
    ...
    let (ai_summaries_res, entity_links_res, session_ratings_res, analysed_ids) = tokio::join!(
        crate::admin::repositories::usage_aggregations::fetch_session_ai_summaries(
            pool,
            &session_ids
        ),
        crate::admin::repositories::conversation_analytics::fetch_all_session_entity_links(
            pool, user_id
        ),
        crate::admin::repositories::conversation_analytics::fetch_all_session_ratings(
            pool, user_id
        ),
    ...
    let entity_links = entity_links_res.unwrap_or_else(|_| Vec::new());
    let session_ratings = session_ratings_res.unwrap_or_else(|_| Vec::new());
