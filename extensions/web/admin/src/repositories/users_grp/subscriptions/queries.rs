...
pub async fn find_subscription_by_user(
    ...
    sqlx::query_as!(
        SubscriptionRow,
        r#"SELECT id, user_id AS "user_id!: UserId", paddle_subscription_id, paddle_customer_id, plan_id, status,
...
pub async fn find_customer_by_paddle_id(
    ...
    sqlx::query_as!(
        PaddleCustomerRow,
        r#"SELECT id, user_id AS "user_id!: UserId", paddle_customer_id, email AS "email!: _", name
...
pub async fn find_user_tier(
    pool: &PgPool,
    user_id: &UserId,
) -> Result<Option<crate::types::UserTier>, sqlx::Error> {
    let row = sqlx::query!(
    ...
    Ok(row.map(|r| crate::types::UserTier {
