...
pub async fn ensure_default_hooks(
    ...
    for event in DEFAULT_HOOK_EVENT_TYPES {
        let event_name = event.as_str();
        if existing.iter().any(|e| e == event_name) {
            continue;
        }
        let id = uuid::Uuid::new_v4().to_string();
        let hook_name = format!("Platform: {event_name}");
        let description = format!("Default platform hook for {event_name} events");
        sqlx::query!(
