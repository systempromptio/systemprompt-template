...
use crate::repositories::export_resolvers::{
...
pub(super) struct PluginBuildContext<'a> {
    pub plugin_id: &'a str,
    pub plugin: &'a crate::types::PlatformPluginConfig,
...
fn build_mcp_files(
    ...
    for mcp_name in &plugin.mcp_servers {
        ...
        mcp_server_entries.insert(
            mcp_name.clone(),
            crate::repositories::export::McpServerEntry {
                server_type: "http".to_string(),
                url,
            },
        );
    }
    let mcp_config = crate::repositories::export::McpConfigFile {
