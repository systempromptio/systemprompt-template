...
use sqlx::PgPool;
use std::path::Path;
use systemprompt::identifiers::{FileId, SkillId, ValidatedFilePath};
...
pub struct SkillFile {
    ...
    pub file_path: ValidatedFilePath,
    pub content: String,
    pub category: String,
    pub language: String,
    pub executable: bool,
...
pub async fn list_skill_files(
    ...
    let rows = sqlx::query_as!(
        SkillFile,
        r#"SELECT id as "id: FileId", skill_id as "skill_id: SkillId",
            file_path as "file_path: ValidatedFilePath",
            content, category, language, executable, size_bytes, checksum, created_at, updated_at
         FROM skill_files WHERE skill_id = $1 ORDER BY category, file_path"#,
        skill_id,
    )
    .fetch_all(pool)
    .await?;
...
pub async fn find_skill_file(
    ...
    let row = sqlx::query_as!(
        ...
        r#"SELECT id as "id: FileId", skill_id as "skill_id: SkillId",
            file_path as "file_path: ValidatedFilePath",
            content, category, language, executable, size_bytes, checksum, created_at, updated_at
         FROM skill_files WHERE skill_id = $1 AND file_path = $2"#,
        skill_id,
        file_path,
    )
    .fetch_optional(pool)
