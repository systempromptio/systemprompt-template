...
pub async fn upsert_skill_secret(
    ...
    let master_key = crate::repositories::secret_crypto::load_master_key().map_err(|e| {
...
pub async fn resolve_secrets_for_skill(
    ...
    for row in &rows {
        ...
        let plaintext = crate::repositories::secret_crypto::decrypt(&dek, &nonce, encrypted_value)
            .map_err(|e| MarketplaceError::Crypto(e.to_string()))?;
