...
use crate::repositories::export::generate_export_bundles;
...
async fn generate_repo(
    ...
    let export_params = crate::repositories::export::ExportParams {
...
fn write_export_to_disk(
    response: &crate::repositories::export::SyncPluginsResponse,
    work_dir: &Path,
    strip_hooks: bool,
...
fn strip_manifest_hooks(content: &str) -> String {
    match serde_json::from_str::<crate::repositories::export::PluginManifest>(content) {
        Ok(mut manifest) => {
            manifest.hooks = None;
...
pub async fn lookup_user_basic(
    pool: &PgPool,
    user_id: &UserId,
) -> Result<crate::types::UserBasicInfo, MarketplaceError> {
    let row = sqlx::query!(
