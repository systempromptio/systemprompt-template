...
use crate::repositories::export::{McpConfigFile, PluginManifest};
use systemprompt::identifiers::{AgentId, McpServerId, SkillId};
use systemprompt_web_shared::error::MarketplaceError;

pub fn import_plugin_bundle(
    services_path: &Path,
    bundle: &crate::repositories::export::PluginBundle,
    ...
    let req = CreatePluginRequest {
        ...
        roles: Vec::new(),
        skills: skill_ids.into_iter().map(SkillId::from).collect(),
        agents: agent_ids.into_iter().map(AgentId::from).collect(),
    ...
    crate::repositories::plugin_crud_ops::create_plugin(services_path, &req)
}
...
fn import_skill_files(
    files: &[crate::repositories::export::PluginFile],
    skills_path: &Path,
...
fn extract_agent_ids(files: &[crate::repositories::export::PluginFile]) -> Vec<String> {
    files
        .iter()
...
fn parse_import_mcp_servers(files: &[crate::repositories::export::PluginFile]) -> Vec<String> {
