...
pub fn find_plugin_detail(
    ...
    let skills: Vec<SkillId> = resolve_all_plugin_skill_ids(&p.base, &skills_path, &agents_path)
        .into_iter()
        .map(SkillId::from)
        .collect();
    Ok(Some(PluginDetail {
        ...
        roles: p.roles,
        skills,
        agents: p.base.agents.include.into_iter().map(AgentId::from).collect(),
        mcp_servers: p
            .base
            .mcp_servers
            .into_iter()
...
fn build_plugin_yaml(req: &CreatePluginRequest) -> String {
    fn yaml_list<T: std::fmt::Display>(items: &[T], indent: &str) -> String {
        if items.is_empty() {
