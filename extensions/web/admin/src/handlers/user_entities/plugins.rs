...
use crate::types::{CreateUserPluginRequest, UpdateUserPluginRequest, UserContext};
use axum::{
...
use crate::handlers::{responses::PluginsListResponse, shared};
...
async fn resolve_user_plugin(
    ...
    user_id: &UserId,
    plugin_id: &str,
) -> Result<crate::types::UserPlugin, Response> {
    match repositories::find_user_plugin(pool, user_id, plugin_id).await {
        Ok(Some(p)) => Ok(p),
        Ok(None) => Err(shared::error_response(
...
async fn mark_dirty_and_record_activity(pool: &Arc<PgPool>, user_id: &UserId, plugin_id: &str) {
