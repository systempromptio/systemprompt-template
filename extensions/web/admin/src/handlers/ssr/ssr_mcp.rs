...
fn build_mcp_server_json(
    ...
    let entity_rules = rules_map.get(m.id.as_str());
...
pub async fn mcp_servers_page(
    ...
    let mcp_servers = if user_ctx.is_admin {
        ...
        let visible_mcp_ids: std::collections::HashSet<McpServerId> = plugins
            .iter()
        ...
        repositories::mcp_servers::list_mcp_servers(&services_path)
            ...
            .into_iter()
            .filter(|m| visible_mcp_ids.contains(m.id.as_str()))
            .collect()
    };
