...
use systemprompt::identifiers::HookId;
...
fn build_plugin_name_map(
    ...
    for entry in entries.flatten() {
        ...
        map.insert(
            plugin_file.plugin.id.to_string(),
...
pub(super) fn list_hooks_from_filesystem(services_path: &std::path::Path) -> Vec<HookDetail> {
    ...
    hooks.sort_unstable_by(|a, b| a.id.as_str().cmp(b.id.as_str()));
    hooks
}
...
pub async fn hook_edit_page(
    ...
    let hook: Option<HookDetail> = hook_id.and_then(|id| {
        ...
        hooks.into_iter().find(|h| h.id.as_str() == id)
    });
