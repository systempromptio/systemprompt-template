...
fn demo_help_admin_pages(page: &str) -> Option<(&'static str, &'static str)> {
    match page {
        "achievements" => Some((
            "<strong>Achievements</strong> displays all available milestones and their unlock status. Achievements are earned by using platform features &mdash; installing plugins, creating skills, running agents, configuring MCP servers, and maintaining usage streaks. Each achievement shows its rarity (how many users have unlocked it), XP reward, and unlock criteria. Achievements encourage feature discovery and consistent platform adoption across teams. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "achievements",
        )),
        "leaderboard" => Some((
            "The <strong>Leaderboard</strong> ranks users by XP, sessions, streaks, and other engagement metrics. It provides a community view of platform adoption, helping identify power users, track team-wide engagement, and encourage healthy competition. Department and team filters let managers see adoption within their groups. Users can opt out of leaderboard visibility in Settings. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "gamification",
        )),
        "settings" => Some((
            "<strong>Settings</strong> manages your account profile, notification preferences, and tier information. It shows your current usage against plan limits &mdash; plugins, skills, agents, MCP servers, hooks, and secrets. Enterprise integrations can customise which settings are available and enforce organisation-wide defaults. Tier upgrades unlock additional capacity and premium features. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "my-workspace",
        )),
        "users" => Some((
            "<strong>Users</strong> is the admin user management console. It shows all registered users with their activity metrics, XP rankings, session counts, roles, departments, and last active timestamps. Admins can create users, assign roles and departments, manage permissions, and monitor adoption across the organisation. User data drives access control policies and governance decisions. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "users",
        )),
        "events" => Some((
            "<strong>Events</strong> displays the system-wide event log &mdash; every Claude tool call, hook execution, session lifecycle event, and platform action across all users. Events can be filtered by type, user, time range, and severity, providing a full audit trail for compliance, debugging, and incident investigation. In production, events integrate with external SIEM systems for enterprise security workflows. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "events",
        )),
        "jobs" => Some((
            "<strong>Jobs</strong> monitors the background job queue &mdash; scheduled tasks including template compilation, analytics aggregation, content pre-rendering, marketplace sync, and activity scoring. Each job shows its cron schedule, last run status, execution duration, and next scheduled run. Failed jobs display error details for debugging. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "jobs",
        )),
        "marketplace" | "marketplace-versions" => Some((
            "The <strong>Marketplace</strong> is the public plugin browser where anyone can discover, evaluate, and install community plugins. Plugins are ranked by usage, ratings, and quality scores. Enterprise teams use the marketplace to publish internal plugins for their organisation, with visibility controls to keep proprietary capabilities private. Version history tracks every change, enabling rollback and audit. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "marketplace",
        )),
        "getting-started" => Some((
            "<strong>Getting Started</strong> helps new users select their initial plugins and configure their workspace. Choose from curated plugin sets based on your role &mdash; developer, analyst, manager, or custom &mdash; then refine your selection. Each plugin set bundles the skills, agents, and integrations most relevant to your workflow. Start by installing the <strong>Claude Code</strong> plugin &mdash; it is the recommended integration for evaluation while Bridge (research preview) stabilises.",
            "getting-started",
        )),
        "traces" => Some((
        ...
        "governance" => Some((
            "<strong>Governance</strong> provides oversight and policy management for AI usage across your organisation. Review every tool call decision (allowed, denied, or modified), configure usage policies per role and department, and set guardrails for how Claude is used by your team. Governance tracks compliance rates, policy violations, and cost allocation in real time. In production, governance policies are enforced automatically at the tool call level. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "tool-governance",
        )),
        "access-control" => Some((
            "<strong>Access Control</strong> manages permissions, roles, and authorisation policies across your organisation. Define which roles and departments can access specific plugins, agents, and MCP servers. Access control operates at the tool call level &mdash; when Claude invokes a governed tool, the platform checks the user's role and department against the configured policies in real time. Audit every permission change with full history. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration while Bridge (research preview) stabilises.",
            "access-control",
        )),
        "setup" => Some((
            "<strong>Setup</strong> guides you through connecting Claude to your workspace. We recommend starting with <strong>Claude Code</strong>, the most stable and feature-complete integration for evaluation &mdash; it supports the full governance pipeline including plugins, hooks, MCP servers, and real-time analytics. Bridge (Desktop) is available as a research preview but has restrictions and shifting APIs that may limit functionality. Configure your webhook endpoint, install the Claude Code plugin with a single command, and verify the connection to start tracking conversations and enabling governance features.",
