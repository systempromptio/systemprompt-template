...
fn demo_help_core_pages(page: &str) -> Option<(&'static str, &'static str)> {
    match page {
        ...
        "profile" => Some((
            "<strong>Profile &amp; Insights</strong> analyses your usage patterns to identify your user archetype, strengths, and areas for improvement. It aggregates 30 days of Claude session data into behavioural analytics, comparing your usage against global averages. This helps teams understand how individuals interact with AI and where additional training or skills could help. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "profile",
        )),
        "dashboard" => Some((
            "The <strong>Admin Dashboard</strong> provides system-wide analytics for platform administrators. It shows web traffic metrics, MCP tool call success rates, user activity timelines, and content performance data. This is the admin-only overview of the entire platform's health and usage. In a production deployment, this dashboard tracks every Claude interaction across your organisation. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "dashboard",
        )),
...
fn demo_help_admin_pages(page: &str) -> Option<(&'static str, &'static str)> {
    match page {
        ...
        "settings" => Some((
            "<strong>Settings</strong> manages your account profile, notification preferences, and tier information. It shows your current usage against plan limits &mdash; plugins, skills, agents, MCP servers, hooks, and secrets. Enterprise integrations can customise which settings are available and enforce organisation-wide defaults. Tier upgrades unlock additional capacity and premium features. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "my-workspace",
        )),
        "users" => Some((
        ...
        "events" => Some((
            "<strong>Events</strong> displays the system-wide event log &mdash; every Claude tool call, hook execution, session lifecycle event, and platform action across all users. Events can be filtered by type, user, time range, and severity, providing a full audit trail for compliance, debugging, and incident investigation. In production, events integrate with external SIEM systems for enterprise security workflows. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "events",
        )),
        "jobs" => Some((
            "<strong>Jobs</strong> monitors the background job queue &mdash; scheduled tasks including template compilation, analytics aggregation, content pre-rendering, marketplace sync, and activity scoring. Each job shows its cron schedule, last run status, execution duration, and next scheduled run. Failed jobs display error details for debugging. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "jobs",
        )),
        "marketplace" | "marketplace-versions" => Some((
            "The <strong>Marketplace</strong> is the public plugin browser where anyone can discover, evaluate, and install community plugins. Plugins are ranked by usage, ratings, and quality scores. Enterprise teams use the marketplace to publish internal plugins for their organisation, with visibility controls to keep proprietary capabilities private. Version history tracks every change, enabling rollback and audit. Connect via <strong>Claude Code</strong> for the best evaluation experience &mdash; it is the recommended integration.",
            "marketplace",
        )),
        "getting-started" => Some((
            "<strong>Getting Started</strong> helps new users select their initial plugins and configure their workspace. Choose from curated plugin sets based on your role &mdash; developer, analyst, manager, or custom &mdash; then refine your selection. Each plugin set bundles the skills, agents, and integrations most relevant to your workflow. Start by installing the <strong>Claude Code</strong> plugin &mdash; it is the recommended integration for evaluation.",
            "getting-started",
        )),
        "traces" => Some((
        ...
        "setup" => Some((
            "<strong>Setup</strong> guides you through connecting Claude to your workspace. We recommend starting with <strong>Claude Code</strong>, the most stable and feature-complete integration &mdash; it supports the full governance pipeline including plugins, hooks, MCP servers, and real-time analytics. Configure your webhook endpoint, install the Claude Code plugin with a single command, and verify the connection to start tracking conversations and enabling governance features.",
