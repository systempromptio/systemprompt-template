...
pub(crate) async fn session_detail_page(
    ...
    let data = json!({
        ...
        "traces": traces.iter().map(trace_json).collect::<Vec<_>>(),
...
fn header_json(h: &SessionHeader) -> Value {
    json!({
        ...
        "started_at_local": h.started_at.map(|t| t.with_timezone(&chrono::Local).format("%Y-%m-%d %H:%M:%S").to_string()),
        "last_activity_at": h.last_activity_at.map(|t| t.to_rfc3339()),
        "last_activity_at_local": h.last_activity_at.map(|t| t.with_timezone(&chrono::Local).format("%Y-%m-%d %H:%M:%S").to_string()),
        "duration_display": duration_display(h.started_at, h.last_activity_at),
        "status": h.status.as_deref().unwrap_or("ended"),
        "model": h.model,
...
fn kpis_json(k: &SessionKpis) -> Value {
    json!({
...
fn context_json(c: &SessionContextRow) -> Value {
    json!({
        ...
        "last_request_at_local": c.last_request_at.map(|t| t.with_timezone(&chrono::Local).format("%Y-%m-%d %H:%M:%S").to_string()),
        "model": c.model,
    })
}

fn trace_json(t: &SessionTraceRow) -> Value {
...
fn request_json(r: &SessionRequestRow) -> Value {
    json!({
        "id": r.id,
        ...
        "context_url": r.context_id.as_ref().map(|c| format!("/admin/contexts/{}", urlencoding::encode(c))),
        "trace_id": r.trace_id,
