...
async fn generate_report_inline(pool: &PgPool) -> DashboardResult<()> {
    use chrono::Timelike;
    ...
    crate::repositories::analytics_grp::dashboard_report::upsert_traffic_report(
        pool,
        today,
        period,
        &report_json,
...
async fn fetch_report_data_inline(pool: &PgPool) -> DashboardResult<InlineReportData> {
    let today = chrono::Utc::now().date_naive();
