...
pub(super) async fn fetch_content_and_breakdown_data(
    pool: &PgPool,
) -> DashboardResult<ContentBreakdownResult> {
...
async fn fetch_top_content(pool: &PgPool) -> DashboardResult<Vec<TopContentRow>> {
    let rows = sqlx::query!(
        r#"
        SELECT
            mc.title as "title!",
...
async fn fetch_seo_metrics(pool: &PgPool) -> DashboardResult<SeoRow> {
    let row = sqlx::query!(
        r#"
        SELECT
...
async fn fetch_geo_breakdown(pool: &PgPool) -> DashboardResult<Vec<GeoRow>> {
    let rows = sqlx::query!(
        r#"
        SELECT
...
async fn fetch_device_breakdown(pool: &PgPool) -> DashboardResult<Vec<DeviceRow>> {
    let rows = sqlx::query!(
        r#"
        SELECT
...
async fn fetch_source_breakdown(pool: &PgPool) -> DashboardResult<Vec<SourceRow>> {
    let rows = sqlx::query!(
        r#"
        SELECT
...
pub(super) async fn fetch_funnel_and_sparklines(
    pool: &PgPool,
) -> DashboardResult<FunnelSparklineResult> {
