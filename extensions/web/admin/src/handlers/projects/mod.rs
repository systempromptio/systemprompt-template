...
use crate::repositories::projects::{crud, members as member_repo};
...
pub(crate) async fn get_project_handler(
    ...
    let member_count = member_repo::count_project_members(&pool, &project_id).await?;
...
pub(crate) async fn create_project_handler(
    ...
    super::groups::validated_id(body.id.as_str())?;
