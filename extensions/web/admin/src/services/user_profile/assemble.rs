//! Per-section assembly for the profile payload.
//!
//! Each function owns one card on the profile pane: the concurrent usage
//! fan-out, the usage view-model, config/identity strings, the gateway access
...
pub(super) async fn fetch_usage_sections(pool: &Arc<PgPool>, user_id: &UserId) -> UsageSections {
    ...
    let (d1, d7, d30, top_models, conversations, identity) = tokio::join!(
        async move {
        ...
        async move {
            find_user_identity(&pool_for_user, &user_id_user)
                .await
                .inspect_err(|e| {
                    tracing::warn!(error = %e, user_id = %user_id_user, "user_profile: find_user_identity failed");
                })
                .ok()
                .flatten()
        }
