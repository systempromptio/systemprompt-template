...
pub fn scan_demos(demo_root: &Path) -> anyhow::Result<DemosConfig> {
    ...
    for meta in CATEGORIES {
        ...
        if !dir.is_dir() {
            eprintln!(
                "demo_scanner: skipping category '{}' — directory missing at {}",
                meta.id,
                dir.display()
            );
            continue;
...
fn extract_commands(content: &str) -> Vec<String> {
    ...
    for line in content.lines() {
        ...
        let cmd = if let Some(args) = trimmed.strip_prefix("run_cli_indented ") {
            Some(format!("systemprompt {}", args.trim()))
        } else if let Some(args) = trimmed.strip_prefix("run_cli_head ") {
            Some(format!("systemprompt {}", args.trim()))
        } else if let Some(args) = trimmed.strip_prefix("run_cli ") {
            Some(format!("systemprompt {}", args.trim()))
        } else if let Some(args) = trimmed.strip_prefix("\"$CLI\" ") {
            Some(format!("systemprompt {}", args.trim()))
