...
pub(super) fn render_social_action_bar(_slug: &str, _title: &str, _org_url: &str) -> String {
    let social_links = [
        ...
        ("twitter", "https://x.com/tyingshoelaces_"),
        ("linkedin", "https://www.linkedin.com/in/edjburton/"),
    ];

    let mut links_html: Vec<String> = social_links
        .iter()
        .filter_map(|(platform, href)| {
            let icon = social_icon(platform);
            if icon.is_empty() {
                return None;
            }
    ...
    let share_icon = social_icon("share");
...
pub(super) fn render_related_posts(posts: &[RelatedPost]) -> Option<String> {
    if posts.is_empty() {
        return None;
    }
...
pub(super) fn render_blog_cards(posts: &[BlogPost]) -> String {
    posts
        .iter()
        .map(|post| {
