...
stage=${1:-all}
case "$stage" in all|unit|contract|integration|registry|boundaries) ;; *) echo "Unknown dashboard test stage: $stage" >&2; exit 2 ;; esac
...
if [[ "$stage" = boundaries ]]; then
    cargo nextest run --locked --manifest-path tests/Cargo.toml -p admin-contract-tests \
        -E 'test(write_boundaries)'
fi
