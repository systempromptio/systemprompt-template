...
- **odoo-pilot**: Skill de `ai-claude-common` para conexion a Odoo via API
- **Credenciales Odoo**: Variables `ODOO_URL`, `ODOO_DB`, `ODOO_KEY`, `ODOO_USER`
- **Node.js**: Para ensamblado JSON en el script de extraccion
- **Python 3**: Para el motor de calculo (`calculate_crm_health.py`)

---

## Instrucciones de Uso

### Paso 0: Recopilacion de Informacion

PREGUNTAR al usuario los siguientes datos:

1. **Credenciales de Odoo** (obligatorio):
   - URL de la instancia (ej: `https://miempresa.odoo.com`)
   - Base de datos
   - API Key o credenciales
   - Si ya tiene las variables de entorno configuradas, confirmar

2. **Objetivo mensual de ventas** (opcional):
   - Revenue target mensual en EUR
   - Si no se proporciona, se omite el indicador de cobertura

3. **Umbral de estancamiento** (opcional, default: 21 dias):
   - Dias sin actividad para considerar una oportunidad estancada
   - Valor por defecto: 21 dias

> **DETENTE** aqui y espera la respuesta del usuario antes de continuar.
