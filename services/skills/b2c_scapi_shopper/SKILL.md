...
> **Note:** For **creating** custom API endpoints, see `b2c_custom_api_development`. This skill focuses on **consuming** standard Shopper APIs.

## Overview

Shopper APIs are designed for frontend commerce applications:

- **Client**: Storefront Next, PWA Kit, composable storefronts, mobile apps
- **Authentication**: SLAS (Shopper Login and API Access Service)
- **Response Time**: < 10 seconds (HTTP 504 if exceeded)
- **CORS**: Not supported - call from a server-side loader/action or BFF (Backend for Frontend), not directly from the browser
...
See `b2c_slas` skill for full client management.
...
- **Security**: Use private SLAS clients only, call from a server-side loader/action or BFF (not browser)
...
const product = await fetch(`...?expand=availability,images,prices`, {
    headers: { Authorization: `Bearer ${accessToken}` },
}).then((r) => r.json());
```
...
- `b2c_slas` - Create and manage SLAS clients
- `b2c_slas_auth_patterns` - Advanced auth: OTP, passkeys, session bridge
- `b2c_scapi_schemas` - Browse OpenAPI schemas
- `b2c_custom_api_development` - Create custom endpoints
- `sfnext_data_fetching` - Consume SCAPI from Storefront Next loaders/actions
