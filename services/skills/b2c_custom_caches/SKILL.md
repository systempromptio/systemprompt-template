...
If **`caches.json` lives only under `cartridge/`** (or any path not referenced from `package.json`) **or** `package.json` omits the **`"caches": "./caches.json"`** field, the runtime **does not register** those cache regions — `CacheMgr.getCache(...)` then fails at runtime with a missing-registration style error. The definition file belongs **next to** `package.json` at the cartridge root, as shown above.
...
- `b2c_custom_api_development` - Cache expensive Custom SCAPI / web service responses
- `b2c_custom_objects` - Persistent data store alternative when caching is not enough
- `b2c_logging` - Read custom error/warn logs for cache diagnostics
