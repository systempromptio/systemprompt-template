...
To download the complete log file, use the `file` field from the JSON output with `b2c_webdav`:
...
| Field       | Description                                                                |
| ----------- | -------------------------------------------------------------------------- |
| `file`      | Source log file name (use with `b2c-webdav` to download full file)         |
...
- `b2c_webdav` - Direct WebDAV file access for downloading full log files
- `b2c_config` - Verify configuration and credentials
- `b2c_logging` - Write logs from cartridge scripts (`Logger`, custom log files)
