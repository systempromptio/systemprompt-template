...
> **Running an existing job?** If you need to execute jobs or import site archives via CLI, use the `b2c_job` skill instead.

## Locating Existing Job Step Definitions

Script to retrieve list of defined job steps in the SFCC project:

```bash
cd <project-root> && node <path-to-this-skill>/scripts/locateJobsDefinition.mjs <optional step name>
```

Returns line number and file path. Use to verify if a job step is defined correctly.

See also: [task step example](references/scriptJobExample.md), [chunk-oriented example](references/chunkJobExample.md)
...
- `b2c_job` - For **running** existing jobs and importing site archives via CLI
- `b2c_webservices` - When job steps need to call external HTTP services or APIs, use the webservices skill for service configuration and HTTP client patterns
