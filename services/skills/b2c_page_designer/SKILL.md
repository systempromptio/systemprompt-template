...
> **Definitions vs. rendering.** The page/component **type definitions** (`experience/**/*.json` meta files and their `.js` type scripts) live on the platform and are **renderer-agnostic** — this is the primary focus of this skill. The `.isml` templates and `dw/util/Template` render functions shown below are the **SFRA / hybrid rendering path**. In **Storefront Next**, Page Designer content is fetched via the Shopper/Experience API and rendered with React **decorators** — see `sfnext_page_designer`.
...
/my-cartridge
    /cartridge
        ...
        /templates                          # SFRA / hybrid rendering path only
            /default
                /experience
                    /pages
                        homepage.isml   # Page template (SFRA/hybrid)
                    /components
                        banner.isml     # Component template (SFRA/hybrid)
```

> The `.json` meta and `.js` type scripts are required on the platform regardless of storefront. The `/templates/**/*.isml` files apply only when rendering through SFRA/hybrid; a Storefront Next app renders the same component definitions with decorators instead.
...
## Rendering Pages (SFRA / hybrid)

> These examples cover server-side rendering through SFRA. For Storefront Next, fetch the page via the Shopper Experience / Page Designer API and render with decorators — see `sfnext_page_designer`.
...
1. **Use `dw.util.Template`** for SFRA/hybrid rendering (NOT `dw.template.ISML`)
...
- `sfnext_page_designer` - Render Page Designer content in Storefront Next via decorators
- `b2c_content` - Content assets, folders, and content slots
- `b2c_metadata` - System/custom object attribute definitions
