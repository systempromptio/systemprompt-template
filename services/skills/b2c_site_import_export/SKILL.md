...
> **Boundary:** This skill owns the **import/export workflow** (CLI, archive layout, job status). For the full **metadata XML schema** — attribute types, flags, enum values, custom object type definitions — see the `b2c_metadata` skill. The XML snippets below are minimal, in-context examples.
...
For service configurations (HTTP, FTP, SOAP services), see the `b2c_webservices` skill which includes:
...
Where `services-folder/services.xml` follows the patterns in the `b2c_webservices` skill.
...
- `b2c_metadata` - The metadata XML schema: attribute types, flags, enum values, custom object definitions
- `b2c_webservices` - Service configurations (HTTP, FTP, SOAP), services.xml format
- `b2c_job` - Running jobs and monitoring import status
