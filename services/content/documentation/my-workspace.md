...
| Issue | Solution |
|-------|----------|
| "No plugins yet" empty state appears | Create your first plugin with **+ Create Plugin** or fork one from the organization with **Fork from Org**. |
| Fork button does not show any organization resources | Your user roles or department may not have access to any organization marketplaces. Contact your administrator to check access control settings. |
| Forked plugin is missing skills or agents | Deep fork copies all associated resources, but individual fork failures are logged as warnings. Check the server logs with `systemprompt infra logs view --level warn` for details. |
| Changes to my resources do not appear in agent sessions | After modifying workspace resources, the system marks your profile as "dirty" for re-sync. If changes still do not appear, try restarting your agent session. |
| Cannot edit the Plugin ID field | Plugin IDs, skill IDs, agent IDs, MCP server IDs, and hook IDs are immutable after creation. To change an ID, delete the resource and create a new one. |
