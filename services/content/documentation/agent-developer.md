...
Developers interact with the Platform Agent through the platform CLI:

```bash
# Check platform agent status
systemprompt infra services status --agent developer_agent

# View platform agent conversation history
...
# Check MCP tool usage by the platform agent
systemprompt analytics tools stats --agent developer_agent

# View agent errors
systemprompt infra logs view --agent developer_agent --level error --since 1h

# Trace failed platform agent interactions
systemprompt infra logs trace list --agent developer_agent --status failed

# List skills available to the platform agent
systemprompt core skills list --agent developer_agent
```
...
Platform Agent interactions tend to be longer and more complex than guest or revenue interactions. Key metrics to monitor:

| Metric | Target | Why |
|--------|--------|-----|
| Conversation length | 5-20 turns | Developer conversations are iterative |
| Tool calls per session | 3-10 | Complex tasks involve multiple tool invocations |
| MCP tool latency | < 1s p95 | Developers expect fast tool responses |
| Security scan completion | < 30s | Scanning should not block development flow |
| Code generation accuracy | > 90% first-pass | Measured by developer acceptance rate |

### Alerting

Configure hooks for platform agent events:

- `PostToolUse` on the `systemprompt` MCP server -- track platform management actions
- `PostToolUseFailure` -- alert on MCP tool failures that block developer workflows
- Custom hooks for deployment events -- ensure human-in-the-loop compliance

## Troubleshooting

**MCP tools not available** -- Verify the `systemprompt` MCP server is running and enabled. Check `systemprompt plugins mcp status systemprompt` for connection status.

**Agent not responding** -- Check that port 9026 is accessible and the agent is enabled. Verify the AI provider has valid credentials with `systemprompt infra logs view --level error --since 1h`.

**Security scan timeouts** -- Security scanning sub-agents may time out on large repositories. Increase the timeout configuration or scope the scan to changed files only.

**Admin scope errors** -- The Platform Agent requires `admin` scope. Verify the OAuth token includes this scope. User-scoped tokens (used by the Revenue Agent) will be rejected.

**Code generation quality issues** -- Check the system prompt for any overriding instructions that may conflict with code quality guidelines. Review `systemprompt infra logs audit <request-id> --full` to see the full conversation context.
