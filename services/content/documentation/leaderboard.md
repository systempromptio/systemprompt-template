...
**Leaderboard is empty** — No users have recorded usage events yet. Usage events are tracked automatically through hooks when users interact with AI capabilities.

**XP seems incorrect** — XP is recalculated from raw event data each time the page loads. If events were recently added or removed, the XP will update on the next page load.

**Department tab shows no data** — Users must have a `department` field set in their user profile. Users without a department are excluded from the department view.

**Streak shows a dash** — The user has no current active streak. They may have been inactive for at least one full calendar day, which resets the current streak to 0.

**User's rank seems low despite high activity** — Check whether the user's events are being tracked under the correct user ID. Also verify that the user exists in the `users` table, since the leaderboard joins against it.
