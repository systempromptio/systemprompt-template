...
**Agent not appearing in the list** -- If you are not an admin, you only see agents assigned to plugins that match your roles. Ask an admin to verify the agent is included in a plugin you have access to.

**Agent not responding to conversations** -- Check that the agent is enabled (toggle is on). Verify the underlying AI provider is configured and has valid credentials. Use `systemprompt infra logs view --level error --since 1h` to check for errors.

**Cannot delete an agent** -- Only admins can delete agents. If you need an agent removed, contact your administrator.

**System prompt changes not taking effect** -- After saving, the updated prompt applies to new conversations. Existing conversations may continue using the previous prompt until they are restarted.
