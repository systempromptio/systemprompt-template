...
**All jobs show "Never run"** — The job scheduler may not be started. Check `systemprompt infra logs view --level error` for scheduler startup errors.

**Job shows "Failed"** — Hover the status badge to see the error message. Common causes include database connection issues, external service timeouts, or configuration problems. Check `systemprompt infra logs view --level error --since 1h` for detailed error logs.

**Jobs page is empty** — No jobs are registered in the database. This may indicate the platform has not been fully initialized. Run `systemprompt infra --help` to check available setup commands.

**Job schedule not changing** — Job schedules are defined in the platform configuration. Changes require updating the configuration and restarting the scheduler.
