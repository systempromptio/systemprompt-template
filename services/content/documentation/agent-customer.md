...
# View recent errors
systemprompt infra logs view --agent customer_agent --level error --since 1h

# View all Guest Agent logs
systemprompt infra logs view --agent customer_agent --since 24h
```

### Performance Monitoring

```bash
# Conversation analytics for Guest Agent
systemprompt analytics conversations --agent customer_agent

# Session metrics
systemprompt analytics sessions --agent customer_agent --since 7d

# Request-level details
systemprompt infra logs request list --agent customer_agent --limit 20
```
...
# Check for tool failures (if MCP servers are configured)
systemprompt infra logs tools list --agent customer_agent --status error
```

## Troubleshooting

**Agent not responding** -- Verify the agent is enabled in `services/agents/customer_agent.yaml` (check `enabled: true`). Confirm the platform is running and port 9029 is not blocked.

**Authentication failures** -- Ensure the OAuth token includes the `admin` scope and the audience matches `a2a`. Check token expiration.

**Slow responses** -- If streaming is enabled but responses feel slow, check the AI provider's latency. Use `systemprompt infra logs request list` to see response times.

**Escalation not working** -- Verify that the escalation webhook or human handoff endpoint is configured. The system prompt defines escalation behavior, but the actual handoff requires infrastructure integration.

**Guest seeing wrong agent** -- Check plugin and role assignments. The guest-facing application should route to `customer_agent` specifically, not to the primary agent.
