...
```bash
# Revenue Agent health
systemprompt infra services status --agent associate_agent

# Error monitoring
systemprompt infra logs view --agent associate_agent --level error --since 1h

# Rate-update-cycle traffic monitoring
systemprompt analytics sessions --agent associate_agent --since 1h

# Property-level usage breakdown
systemprompt analytics conversations --agent associate_agent

# Failed interactions
systemprompt infra logs trace list --agent associate_agent --status failed
```

## Troubleshooting

**Agent slow during rate update cycle** -- This is expected under high concurrency. Verify auto-scaling is configured and instances are spinning up ahead of the rate update window.

**Revenue manager seeing wrong property data** -- Check the OAuth token claims for role and portfolio. Verify MCP tool scoping matches the revenue manager's actual property assignment.

**Rate data stale** -- The RMS integration sub-agent may be caching data. Check the MCP server connection status and data freshness settings.

**Occupancy forecasts incorrect** -- Real-time occupancy depends on the PMS integration. Check `systemprompt infra logs tools list --agent associate_agent --status error` for MCP tool failures.

**Revenue manager cannot access the agent** -- Verify the revenue manager has a valid `user`-scoped token. Check that the `enterprise-demo` plugin is active and the revenue manager's role includes access.
