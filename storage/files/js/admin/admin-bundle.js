...
(function(app) {
    ...
    function renderPluginDetails(plugin, activeTab) {
        ...
        let tabs = '';
        tabs += '<button type="button" class="sp-tab' + (activeTab === 'skills' ? ' sp-tab--active' : '') + '" role="tab" aria-selected="' + (activeTab === 'skills' ? 'true' : 'false') + '" data-tab="skills" data-plugin-owner="' + escapeHtml(plugin.id) + '">Skills <span class="sp-tab-count">' + skillCount + '</span></button>';
        tabs += '<button type="button" class="sp-tab' + (activeTab === 'agents' ? ' sp-tab--active' : '') + '" role="tab" aria-selected="' + (activeTab === 'agents' ? 'true' : 'false') + '" data-tab="agents" data-plugin-owner="' + escapeHtml(plugin.id) + '">Agents <span class="sp-tab-count">' + agentCount + '</span></button>';
        tabs += '<button type="button" class="sp-tab' + (activeTab === 'mcp' ? ' sp-tab--active' : '') + '" role="tab" aria-selected="' + (activeTab === 'mcp' ? 'true' : 'false') + '" data-tab="mcp" data-plugin-owner="' + escapeHtml(plugin.id) + '">MCP Servers <span class="sp-tab-count">' + mcpCount + '</span></button>';
        tabs += '<button type="button" class="sp-tab' + (activeTab === 'hooks' ? ' sp-tab--active' : '') + '" role="tab" aria-selected="' + (activeTab === 'hooks' ? 'true' : 'false') + '" data-tab="hooks" data-plugin-owner="' + escapeHtml(plugin.id) + '">Hooks <span class="sp-tab-count">' + hookCount + '</span></button>';
        let content = '';
        ...
        return '<div style="border-top:1px solid var(--sp-border-subtle);padding:var(--sp-space-4)">' +
            '<div class="sp-tabs" role="tablist" style="display:flex;gap:var(--sp-space-2);margin-bottom:var(--sp-space-4)">' + tabs + '</div>' +
            '<div class="sp-tab-panel">' + content + '</div>' +
        '</div>';
    }
    app.pluginDetails = {
...
(function(app) {
    ...
    function initTabs() {
        const buttons = document.querySelectorAll('.sp-tab');
        ...
        buttons.forEach((btn) => {
            btn.addEventListener('click', () => {
                buttons.forEach((b) => { b.classList.remove('sp-tab--active'); });
