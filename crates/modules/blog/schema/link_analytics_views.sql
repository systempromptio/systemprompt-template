-- View: Campaign link performance with aggregated click data
...
-- View: Content journey map (internal navigation flow)
...
-- View: Link click stream (recent activity)
...
-- View: Top performing links (by clicks and conversions)
...
-- View: Link performance by device type
CREATE OR REPLACE VIEW v_link_performance_by_device AS
SELECT
    cl.id as link_id,
    cl.short_code,
    cl.campaign_name,
    ...
    ROUND(100.0 * COUNT(CASE WHEN lc.is_conversion THEN 1 END) / NULLIF(COUNT(*), 0), 2) as conversion_rate
FROM campaign_links cl
JOIN link_clicks lc ON cl.id = lc.link_id
WHERE lc.device_type IS NOT NULL
GROUP BY cl.id, cl.short_code, cl.campaign_name, lc.device_type
ORDER BY click_count DESC;

-- View: Link performance by country
CREATE OR REPLACE VIEW v_link_performance_by_country AS
SELECT
    cl.id as link_id,
    cl.short_code,
    cl.campaign_name,
...
FROM campaign_links cl
JOIN link_clicks lc ON cl.id = lc.link_id
WHERE lc.country IS NOT NULL
GROUP BY cl.id, cl.short_code, cl.campaign_name, lc.country
ORDER BY click_count DESC;

-- View: Campaign performance over time (daily aggregation)
...
SELECT
    ...
    COUNT(*) as clicks,
    COUNT(DISTINCT lc.session_id) as unique_sessions,
    COUNT(CASE WHEN lc.is_first_click THEN 1 END) as first_clicks,
    COUNT(CASE WHEN lc.is_conversion THEN 1 END) as conversions,
    ROUND(100.0 * COUNT(CASE WHEN lc.is_conversion THEN 1 END) / NULLIF(COUNT(DISTINCT lc.session_id), 0), 2) as conversion_rate
FROM campaign_links cl
JOIN link_clicks lc ON cl.id = lc.link_id
WHERE cl.campaign_id IS NOT NULL
GROUP BY cl.campaign_id, cl.campaign_name, DATE(lc.clicked_at)
ORDER BY date DESC;

-- View: Source content performance (which blog posts drive most clicks)
CREATE OR REPLACE VIEW v_source_content_performance AS
SELECT
    mc.id as content_id,
    mc.slug,
    mc.title,
...
JOIN campaign_links cl ON mc.id = cl.source_content_id
LEFT JOIN link_clicks lc ON cl.id = lc.link_id
GROUP BY mc.id, mc.slug, mc.title, mc.source_id
HAVING SUM(cl.click_count) > 0
ORDER BY total_clicks DESC;

-- Indexes on link_clicks for better view performance
...
CREATE INDEX IF NOT EXISTS idx_link_clicks_date ON link_clicks(DATE(clicked_at));
